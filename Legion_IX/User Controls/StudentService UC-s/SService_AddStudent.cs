...
namespace Legion_IX.User_Controls.StudentService_UC_s
{
    internal partial class UC_StudentService_ADMIN
    {
        ...
        private bool InputValidation_Student(object sender, EventArgs e)
        {
            ...
            // EMAIL VALIDATION
            if (!MyValidators.ValidateEmail(txtBox_StudentEmail.Text))
                err_EmailFormat.SetError(txtBox_StudentEmail, "Wrong Email Format!");

            else
            {
                err_EmailFormat.SetError(txtBox_StudentEmail, "");
                AllGood = true;
            }

            // PASSWORD VALIDATION
            if (!MyValidators.ValidatePassword(txtBox_StudentEmail.Text))
                err_PasswordFormat.SetError(txtBox_StudentEmail, "Wrong Password Format!");

            else
            {
                err_PasswordFormat.SetError(txtBox_StudentEmail, "");
                AllGood = true;
            }
        ...
        private void btn_CreateStudent_Click(object sender, EventArgs e)
        {
            if (InputValidation_Student(sender, e))
            {
                ...
                if (newStudent.CreateAndUpload())
                {
                    ...
                    txtBox_Index.Text = "";
                    txtBox_StudentPassword.Text = "";
                    picBox_StudentPhoto.Image = null;
                }
